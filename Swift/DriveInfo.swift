...
/// The type of drive for a trip, as returned by the `getInternalDriveType` API
@objc(ZDInternalDriveTypePreview) public enum InternalDriveTypePreview: Int32 {
    /// something is wrong (SDK not set up, no trip found)
    case invalid = -1

    /// traveler was traveling in a car (we are not sure of the driver passenger status)
    case drive
    ...
    /// traveler was traveling in a train
    case transit

    /// traveler was traveling in a high speed vehicle, e.g.- train, airplane etc
    case highSpeedVehicle
    ...
    /// traveler was not on road/rail network (e.g., airplane, boat/ferry, golf cart, etc.)
    case offRoad

    func toObjcInternalDriveTypePreview() -> ZendriveSDK.ZendriveInternalDriveTypePreview {
        ZendriveSDK.ZendriveInternalDriveTypePreview(rawValue: rawValue) ?? ZendriveSDK.ZendriveInternalDriveTypePreview.invalid
    }

    static func fromObjcInternalDriveTypePreview(_ objcInternalDriveTypePreview: ZendriveSDK.ZendriveInternalDriveTypePreview) -> InternalDriveTypePreview {
        InternalDriveTypePreview(rawValue: objcInternalDriveTypePreview.rawValue) ?? InternalDriveTypePreview.invalid
    }
}
...
@objc(ZDDriveInfo) public class DriveInfo: NSObject {
    ...
    /// If `driveType` is not `DriveType.drive` then it will be set to nil.
    @objc public var extrapolationDetails: ExtrapolationDetails?

    /// The vehicle tagging details of this drive
    /// Available only if a drive is tagged with vehicle. Otherwise it is nil.
    @objc public var vehicleTaggingDetails: VehicleTaggingDetails?
    ...
    @objc override convenience init() {
     ...
     init(with objcDriveInfo: ZendriveSDK.ZendriveDriveInfo) {
        ...
        self.extrapolationDetails = ExtrapolationDetails(with: objcDriveInfo.extrapolationDetails)
        self.vehicleTaggingDetails = VehicleTaggingDetails(with: objcDriveInfo.vehicleTaggingDetails)
    }
    ...
    func toObjcDriveInfo() -> ZendriveDriveInfo {
        ...
        objcDriveInfo.extrapolationDetails = extrapolationDetails?.toObjcExtrapolationDetails()
        objcDriveInfo.vehicleTaggingDetails = vehicleTaggingDetails?.toObjcVehicleTaggingDetails()
