...
@objc(ZDZendriveError) public enum ZendriveError: Int {
    ...
    /// pausedTillTimestamp parameter passed to the `Zendrive.pauseAutoTracking(_:)` API must be in future
    case expiredPausedTillTimestamp = 116

    /// this error is thrown when `Zendrive.refreshBusinessHours(_:)` is timed out.
    case refreshBusinessHoursTimeout = 117

    /// this error is thrown in case of network unavailability when `Zendrive.refreshBusinessHours(_:) ` is called.
    case refreshBusinessHoursNetworkNotAvailable = 118

    /// this error is thrown when `Zendrive.refreshBusinessHours(_:)` is called but business hours feature is not enabled.
...
extension ZendriveError: LocalizedError {
    public var errorDescription: String? {
        ...
        case .invalidBeaconUUID:
            return "The UUID string of this beacon is invalid."
        case .beaconNotFound:
            return "This beacon was never added. Please check the details and try again."
        case .maxBeaconLimitReached:
            return "Cannot add more than 2 beacons."
        case .expiredPausedTillTimestamp:
            return "Invalid pausedTillTimestamp as it must be in future"
        case .refreshBusinessHoursTimeout:
            return "Business hours timed out"
        case.refreshBusinessHoursNetworkNotAvailable:
            return "refresh business hours call failed due to network unavailability"
        case.refreshBusinessHoursFeatureNotEnabled:
            return "Business hours feature is not enabled"
        @unknown default:
