...
@objc(ZDZendrive) public final class Zendrive: NSObject {
    ...
    private static var zendriveDelegate: ZendriveSDK.ZendriveDelegateProtocol?
    ...
    ///
    /// Things to Consider:-
    ///  * To change the `Configuration.region`, you need to call `+wipeOut:` first.
    ...
    ///    it may get torndown. In all such cases please check `isSDKSetup` in the completion
    ///    handler to confirm if SDK is setup or not.
    ///  * If `Configuration.driverId` or `Configuration.applicationKey` is changed while
    ...
    /// - Parameter sdkHealthReason: indicates the reason for logging SDK Health
    ///
    ///
    @objc public static func logSDKHealth(_ reason: ZendriveSDKHealthReason, completionHandler:  @escaping (Error?) -> Void ) {
        ZendriveSDK.Zendrive.logSDKHealth(reason) { error in
            let swiftError = ZendriveError.fromError(error)
            completionHandler(swiftError)
        }
    }
    ...
    /// - Parameter timestamp: timestamp of the trip
    /// - Returns: internal drive type enum
    @objc public static func getInternalDriveType(forTimestamp timestamp: Int64) -> InternalDriveTypePreview {
        InternalDriveTypePreview.fromObjcInternalDriveTypePreview(ZendriveSDK.Zendrive.getInternalDriveType(forTimestamp: timestamp))
    }
     ...
     *
     * - Parameter  pausedTillTimestamp - till the pausedTillTimestamp auto tracking will be paused
     *
     * Example:
     *
     * ```
    ...
    @objc public static func pauseAutoTracking(_ pausedTillTimestamp: Int64) throws {
        do {
            try ZendriveSDK.Zendrive.pauseAutoTracking(pausedTillTimestamp)
        } catch let error {
            throw ZendriveError.fromError(error) ?? error
        }
    }
    ...
    @objc public static func resumeAutoTracking() throws {
        do {
            try ZendriveSDK.Zendrive.resumeAutoTracking()
        } catch let error {
            throw ZendriveError.fromError(error) ?? error
        }
    }
    ...
    @objc public static func isAutoTrackingPaused() throws -> NSNumber {
        do {
            return try ZendriveSDK.Zendrive.isAutoTrackingPaused()
        } catch {
            throw ZendriveError.fromError(error) ?? error
        }
    }
    ...
    /// throws: `ZendriveError.notSetup`
    /// - Returns: PausedReason encapsulating ZendrivePausedReasonEnum with values.
    ...
    public static func getPausedReason() throws -> PausedReason {
        do {
            let objcpausedReason = try ZendriveSDK.Zendrive.getPausedReason()
            return PausedReason(with: objcpausedReason)
        } catch let error {
            throw ZendriveError.fromError(error) ?? error
        }
    }
    ...
    /// shiftDetails, A nullable `ZendriveShiftDetail` object which provides the business hours.
    /// error.
    /// possible error codes returned:
    /// ```
    ///  ZendriveError.notSetup,
    ///  ZendriveError.refreshBusinessHoursTimeout,
    ///  ZendriveError.refreshBusinessHoursNetworkNotAvailable,
    ///  ZendriveError.refreshBusinessHoursFeatureNotEnabled
    /// ```
    /// Example:
    /// ```
    /// Zendrive.refreshBusinessHours { (shiftDetail, error) in
    ///     if let error = error {
    ///         // handle error
    ///     } else {
    ...
    @objc public static func refreshBusinessHours(completionHandler:  @escaping (_ businessHours: ShiftDetail?, _ error: NSError?) -> Void ) {
        ZendriveSDK.Zendrive.refreshBusinessHours { businessHours, error in
            if let businessHours = businessHours {
                let businessHoursSwift = ShiftDetail(shiftName: businessHours.shiftName, shiftId: businessHours.shiftId, timezone: businessHours.timezone, shiftIntervals: ShiftInterval.convertArray(businessHours.shiftIntervals))
                completionHandler(businessHoursSwift, nil)
            } else if error != nil {
                let swiftError = ZendriveError.fromError(error) ?? error
                completionHandler(nil, swiftError as NSError?)
            } else {
                // This will never happen, but we should just make sure that the client completion handler is always called.
...
@objc(ZDZendriveDelegate) public protocol ZendriveDelegate: NSObjectProtocol {
    ...
    /// NOTE: To fetch the businessHours instantly use `Zendrive.refreshBusinessHours(_:)`
    /// - Parameter businessHours, a valid 'ShiftDetail' object that contains  the new business hours that are applied by the
    /// fleet manager.
    ///
    @objc optional func businessHoursChanged(_ businessHours: ShiftDetail)
}
...
private class DelegateImpl: NSObject, ZendriveSDK.ZendriveDelegateProtocol {
    ...
    func businessHoursChanged(_ businessHours: ZendriveShiftDetail) {
        delegate?.businessHoursChanged?(ShiftDetail(with: businessHours))
    }
}
